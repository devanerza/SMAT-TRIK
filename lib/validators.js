...
export function validateOrderItem(item) {
  ...
  const validCapacities = ['0.5 PK', '0.75 PK', '1 PK', '1.5 PK', '2 PK', '2.5 PK'];
 ...
 * - custLat: optional, but if provided, must be a valid latitude (-90 to 90).
 * - custLng: optional, but if provided, must be a valid longitude (-180 to 180).
...
export function validateCustomerInfo(customerInfo) {
  ...
  const { custName, custPhone, custLocUrl, custLat, custLng, custEmail } = info;
  ...
  if (custLat !== undefined && custLat !== null) {
    if (typeof custLat !== 'number' || isNaN(custLat) || custLat < -90 || custLat > 90) {
      errors.custLat = 'Latitude tidak valid';
    }
  }

  if (custLng !== undefined && custLng !== null) {
    if (typeof custLng !== 'number' || isNaN(custLng) || custLng < -180 || custLng > 180) {
